...
use crate::{client::get_latest_header, compression::compress, taiko::contracts::TaikoInbox};
...
pub async fn get_header_info_stream(
                ...
                StreamData::Unconfirmed(block) => {
                    ...
                    if let Some(latest_batch_proposed) = latest_batch_proposed.read().await.clone() {
                        ...
                        if block.header.number > latest_batch_proposed.info.lastBlockId {
                            let info = HeaderInfo::new(block.header.number, block.header.timestamp);
                ...
                StreamData::Confirmed(batch_proposed) => {
                    ...
                    let info = {
                        ...
                        let info = if unconfirmed_l2_blocks.is_empty() {
                            Some(HeaderInfo::new(batch_proposed.info.lastBlockId, batch_proposed.info.lastBlockTimestamp))
                        } else {
                            None
                        };
...
mod tests {
    ...
    use crate::{
        ...
        test_util::{get_header, get_header_with_mixhash},
    };
    ...
    fn get_test_signed_transaction(nonce: u64) -> Transaction<TxEnvelope> {
        ...
        //        let signed = get_signed(&signing_key, tx).unwrap();
    ...
    async fn header_info_stream_returns_info_from_l2_blocks_if_no_confirmed_blocks_are_yet_available()
     {
        ...
        let stream = get_header_info_stream(
            l2_block_stream,
            confirmation_stream,
        ...
        assert_eq!(received, Some(HeaderInfo::new(1, 3)));
        let received = stream.next().await;
        assert_eq!(received, Some(HeaderInfo::new(2, 5)));
        assert_eq!(
    ...
    async fn header_info_stream_returns_info_from_l2_blocks_only_if_block_number_is_bigger_than_confirmed_l1_block()
     {
        ...
        pin_mut!(stream);
        let received = stream.next().await;
        assert_eq!(received, Some(HeaderInfo::new(2, 2)));
        let received = stream.next().await;
        assert_eq!(received, Some(HeaderInfo::new(3, 3)));
        let received = stream.next().await;
        assert_eq!(received, None);
        assert_eq!(
            *unconfirmed_l2_blocks.read().await,
    ...
    async fn header_info_stream_removes_all_blocks_from_unconfirmed_blocks_if_txs_cause_mismatch() {
        ...
        let received = stream.next().await;
        assert_eq!(received, Some(HeaderInfo::new(1, 2)));
        assert_eq!(
            *unconfirmed_l2_blocks.read().await,
            final_expected_unconfirmed_l2_blocks
        );
